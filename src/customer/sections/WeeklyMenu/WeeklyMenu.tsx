...
export const WeeklyMenu: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedPlan && menuId) {
      loadRecipesForPlan(selectedPlan, menuId);
    }
  }, [selectedPlan, menuId]);
  ...
  const loadRecipesForPlan = async (planId: string, mId: string) => {
    try {
      ...
      const recipesMap = new Map<string, RecipeInfo>();
      ...
      <div className="max-w-3xl mx-auto px-4 py-24 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-3">
          {error === 'no_menu' || error === 'no_recipes'
            ? 'El menu de la proxima semana aun no esta disponible'
            : 'Hubo un problema al cargar el menu'}
        </h2>
