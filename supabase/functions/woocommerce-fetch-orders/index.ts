...
async function processOrder(
  ...
  try {
    ...
    if (mappings.length === 0) {
      await markFailed("No se encontraron mapeos activos para los productos: " + allProductIds.join(", "));
      return;
    }
    ...
    for (const lineItem of wcOrder.line_items) {
      ...
      for (let i = 0; i < numWeeks; i++) {
        ...
        if (owErr) {
          console.error(`Error creating order_week for product ${lineItem.product_id}:`, owErr);
        }
      }
    }
    ...
    console.error("WooCommerce order processing error:", msg);
    await supabase
      .from("woocommerce_order_imports")
      .update({ status: "failed", error_message: msg })
      .eq("id", importId);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const perPage = Math.min(Number(body.per_page) || 10, 50);
    const page = Number(body.page) || 1;
    const statusFilter: string = body.status || "processing";

    const { data: config, error: configErr } = await supabase
      .from("woocommerce_config")
      .select("id, store_url, consumer_key, consumer_secret, webhook_secret, cutoff_day, cutoff_hour, test_mode")
      .eq("is_active", true)
      .maybeSingle() as { data: WooConfig | null; error: unknown };

    if (configErr || !config) {
      return new Response(JSON.stringify({ error: "No se encontró configuración de WooCommerce activa" }), {
        status: 400,
    ...
    if (!config.store_url || !config.consumer_key || !config.consumer_secret) {
      return new Response(JSON.stringify({ error: "Credenciales de WooCommerce incompletas" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const apiUrl = `${storeUrl}/wp-json/wc/v3/orders?status=${statusFilter}&per_page=${perPage}&page=${page}`;
    ...
    try {
      ...
      if (!wcResponse.ok) {
        ...
        return new Response(JSON.stringify({
          error: `Error de WooCommerce API (${wcResponse.status}): ${errorText.slice(0, 300)}`,
        }), {
          status: 502,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      ...
      wcOrders = await wcResponse.json();
    } catch (fetchErr) {
      return new Response(JSON.stringify({
        error: `No se pudo conectar a la tienda WooCommerce: ${fetchErr instanceof Error ? fetchErr.message : String(fetchErr)}`,
      }), {
        status: 502,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    ...
    if (!Array.isArray(wcOrders)) {
      return new Response(JSON.stringify({ error: "La API de WooCommerce devolvió un formato inesperado" }), {
        status: 502,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    for (const wcOrder of wcOrders) {
      ...
      if (existingImport) {
        ...
        if (insertErr) {
          errors.push(`Pedido #${wcOrderNumber}: error al registrar`);
          continue;
        }
        newCount++;
      }
    ...
    console.error("woocommerce-fetch-orders error:", err);
    return new Response(JSON.stringify({
